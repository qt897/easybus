...
function ComingSoon({ label }: { label: string }) {
  return (
    <div className="flex h-full flex-col items-center justify-center gap-2 px-6 text-center text-muted-foreground">
      ...
      <p className="text-xs">Tính năng sẽ sớm ra mắt.</p>
    </div>
  );
}
...
export function Sidebar() {
  ...
  return (
    <aside
      className={cn(
        "fixed inset-x-0 bottom-0 z-20 flex flex-col rounded-t-2xl border-t border-border bg-background shadow-2xl transition-[height] duration-300 ease-out",
        isOpen ? "h-[78vh]" : "h-[152px]",
        "md:static md:z-auto md:h-auto md:w-[380px] md:shrink-0 md:rounded-none md:border-t-0 md:border-r md:shadow-none"
      )}
    >
      <button
        type="button"
        ...
        aria-label={isOpen ? "Thu gọn danh sách" : "Mở rộng danh sách"}
      >
      ...
      >
        ...
        <TabsContent value="directions" className="min-h-0 flex-1">
          <ComingSoon label="Chỉ đường" />
        </TabsContent>

        <TabsContent value="favorites" className="min-h-0 flex-1">
          <ComingSoon label="Tuyến yêu thích" />
        </TabsContent>
      </Tabs>
    </aside>
  );
